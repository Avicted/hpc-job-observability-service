...
func jobRowValues(id string, state JobState, start time.Time, end *time.Time) []driver.Value {
...
func TestPostgresStorage_CreateJob_Duplicate(t *testing.T) {
	...
	err = store.CreateJob(testAuditContext(), &Job{ID: "job-1"})
...
func TestPostgresStorage_CreateJob_InsertError(t *testing.T) {
	...
	store := &PostgresStorage{baseStorage: baseStorage{db: db}}
	job := &Job{ID: "job-2", User: "alice", Nodes: []string{"node-1"}, State: JobStateRunning, StartTime: time.Now()}
...
func TestPostgresStorage_CreateJob_ExistsQueryError(t *testing.T) {
	...
	err = store.CreateJob(testAuditContext(), &Job{ID: "job-err"})
...
func TestPostgresStorage_GetJob_WithNulls(t *testing.T) {
	...
	rows := sqlmock.NewRows(jobRowColumns()).AddRow(jobRowValues("job-3", JobStateRunning, time.Now(), nil)...)
...
func TestPostgresStorage_DeleteJob_ExecError(t *testing.T) {
	...
	rows := sqlmock.NewRows(jobRowColumns()).AddRow(jobRowValues("job-err", JobStateRunning, time.Now(), nil)...)
...
func TestPostgresStorage_GetJobMetrics_NotFound(t *testing.T) {
	...
	_, _, err = store.GetJobMetrics(context.Background(), "missing", MetricsFilter{Limit: 10})
	if !errors.Is(err, ErrJobNotFound) {
		t.Fatalf("expected ErrJobNotFound, got %v", err)
	}
...
func TestPostgresStorage_DeleteJob_Success(t *testing.T) {
	...
	rows := sqlmock.NewRows(jobRowColumns()).AddRow(jobRowValues("job-1", JobStateRunning, time.Now(), nil)...)
...
func TestPostgresStorage_ListJobs(t *testing.T) {
	...
	user := "alice"
	node := "node-1"
	filter := JobFilter{State: &state, User: &user, Node: &node, Limit: 5, Offset: 1}
	...
	jobRows := sqlmock.NewRows(jobRowColumns()).AddRow(jobRowValues("job-1", JobStateRunning, time.Now(), nil)...)
...
func TestPostgresStorage_ListJobs_LimitDefaultAndCap(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, _, err = store.ListJobs(context.Background(), JobFilter{Limit: tc.limit})
			if err != nil {
				t.Fatalf("ListJobs error: %v", err)
			}
...
func TestPostgresStorage_RecordMetrics(t *testing.T) {
	...
	sample := &MetricSample{JobID: "job-1", CPUUsage: 10, MemoryUsageMB: 256}
...
func TestPostgresStorage_GetJobMetrics(t *testing.T) {
	...
	filter := MetricsFilter{StartTime: &start, EndTime: &end, Limit: 2}
...
func TestPostgresStorage_GetJobMetrics_LimitDefault(t *testing.T) {
	...
	_, _, err = store.GetJobMetrics(context.Background(), "job-1", MetricsFilter{Limit: 0})
	if err != nil {
		t.Fatalf("GetJobMetrics error: %v", err)
	}
